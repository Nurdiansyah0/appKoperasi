...
export default function KasirSidebar({ isOpen, setIsOpen }: SidebarProps) {
    const pathname = usePathname();
    const router = useRouter();
    const { user, logout } = useAuth(); // Use context for consistent auth handling

    const menuItems = [
        { name: 'Dashboard', href: '/kasir/dashboard', icon: LayoutDashboard },
        { name: 'Transaksi', href: '/kasir/transaksi', icon: ShoppingCart },
        { name: 'Riwayat', href: '/kasir/riwayat', icon: History },
        { name: 'Keuangan', href: '/kasir/keuangan', icon: Wallet },
        { name: 'Riwayat Opname', href: '/kasir/opname/riwayat', icon: History },
    ];
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-xs text-slate-500">Kasir Panel</p>
                            </div>
                            <button
                                onClick={() => setIsOpen(false)}
                ...
                <div className="p-4 border-t border-slate-100">
                    <div className="flex items-center gap-3 p-2 mb-4 bg-slate-50 rounded-xl">
                        ...
                        <div className="overflow-hidden">
                            <p className="text-sm font-bold text-slate-800 truncate">{user?.username || 'Kasir'}</p>
                            <p className="text-[10px] text-slate-500 uppercase tracking-wider">{user?.role || 'Staff'}</p>
                        </div>
                    </div>
                    <button
                        onClick={logout}
