...
class kasir
{
    ...
    public static function simpanTransaksiKasir($conn)
    {
        ...
        try {
            ...
            ");
                ...
                ");
                    if (!$stmtUpdate) throw new Exception("Prepare update transaksi gagal: " . $conn->error);
    ...
    {
        $user = getAuthUser(self::$jwt_secret);
        $input = $_POST; // Gunakan $_POST untuk menghindari getJsonInput issue
    ...
    public static function getTransaksiTerbaru($conn)
    {
        ...
        error_log("Getting recent transactions for kasir ID: " . $current_user_id);

        // PERBAIKAN: Gunakan nama kolom yang benar - ganti 'a.nama_anggota' dengan 'a.nama_lengkap'
        $sql = "SELECT 
        ...
        $stmt = $conn->prepare($sql);
        if (!$stmt) {
            error_log("Prepare failed for recent transactions: " . $conn->error);
            sendResponse(false, "Query preparation failed");
            return;
        }
        ...
        error_log("Found " . count($transactions) . " recent transactions");
